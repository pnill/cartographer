...
{
	s_biped_data_definition* playerUnit = object_get_fast_unsafe<s_biped_data_definition>(object_index);

	if (player)
		return s_player::GetPlayer(DATUM_INDEX_TO_ABSOLUTE_INDEX(object_index))->identifier;
	else
	{
		short player_index = DATUM_INDEX_TO_ABSOLUTE_INDEX(playerUnit->controlling_player_index);
		return s_player::GetPlayer(player_index)->identifier;
	}
}
...
{
	PlayerIterator playersIt;
	return playersIt.get_data_at_datum_index(player_datum)->unit_index;
}
...
datum VariantPlayer::GetUnitDatum(XUID xuid)
{
	if (xuid_to_unit_datum.find(xuid) == xuid_to_unit_datum.end())
	{
		...
		while (playersIt.get_next_active_player())
		{
			if (playersIt.get_current_player_data()->identifier == xuid)
			{
				datum unit_datum = playersIt.get_current_player_data()->unit_index;
				datum player_datum = DATUM_INDEX_TO_ABSOLUTE_INDEX(playersIt.get_current_player_index());
