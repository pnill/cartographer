...
void GunGame::spawnPlayerServer(int playerIndex) {
	...
	XUID localPlayerIdentifier = getPlayerXuid(h2mod->get_player_datum_index_from_controller_index(0).ToAbsoluteIndex());
	...
	if (unit_object) {
		...
		if (level == 15) {
			...
			if (playerIdentifier == localPlayerIdentifier) {
