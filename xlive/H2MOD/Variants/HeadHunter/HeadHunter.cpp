...
void HeadHunter::SpawnSkull(datum unit_datum)
{
	...
	if (biped_unit != NULL)
	{
		...
		objects::create_new_placement_data(&nObject, e_weapons_datum_index::ball, -1, 0);

		nObject.position = biped_unit->position;
		nObject.translational_velocity = biped_unit->translational_velocity;

		datum new_object_datum = objects::object_new(&nObject);
		if (!DATUM_IS_NONE(new_object_datum))
			objects::simulation_action_object_create(new_object_datum);
	}
}
