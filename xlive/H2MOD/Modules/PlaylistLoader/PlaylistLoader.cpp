...
namespace playlist_loader
{
	...
	bool process_custom_setting(playlist_entry* playlist_entry)
	{
		...
		auto result = false;
		const auto property_name = static_cast<wchar_t*>(calloc(32, sizeof(wchar_t)));
		const auto property_value = static_cast<wchar_t*>(calloc(32, sizeof(wchar_t)));
		const auto temp_name = static_cast<wchar_t*>(calloc(32, sizeof(wchar_t)));
		const auto variant = static_cast<wchar_t*>(calloc(32, sizeof(wchar_t)));

		//Pull the current property name from the buffer.
		for (auto i = 0; i < 32; i++)
		{
			property_name[i] = playlist_entry->section_buffer[68 * playlist_entry->section_buffer_current_index + i];
			if (property_name[i] == 0) break;
		}
		//Pull the current property value from  the buffer.
		for (auto i = 0; i < playlist_entry->reader_current_char_index; i++)
		{
			property_value[i] = playlist_entry->section_buffer[68 * playlist_entry->section_buffer_current_index + 32 + i];
			if (property_value[i] == 0) break;
		}

		//Trim the end of the value incase there is extra whitespace
		for (auto i = playlist_entry->reader_current_char_index; i > 0; i--)
		{
			if (property_value[i] != ' ' || property_value[i] != '\n')
				break;
			property_value[i] = 0;
		}
		...
		{
			for (auto j = 0; j < 32; j++)
				temp_name[j] = playlist_entry->section_buffer[68 * i + j];
			...
			case property_value_read:
				if (*string_buffer == '\n')
				{
					if (!process_custom_setting(playlist_entry)) {
						process_setting(playlist_entry);
					}
