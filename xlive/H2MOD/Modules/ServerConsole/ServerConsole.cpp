...
void* __cdecl dediCommandHook(wchar_t** command_line_args, int split_strings, char a3) {
	...
	EventHandler::execute_callback<EventHandler::ServerCommandEvent>(server_command, execute_before, ServerConsole::s_commandsMap[LowerCommand]);
	auto res = p_dedi_command_hook(command_line_args, split_strings, a3);
	EventHandler::execute_callback<EventHandler::ServerCommandEvent>(server_command,execute_after, ServerConsole::s_commandsMap[LowerCommand]);
	return res;
}
...
bool __cdecl kablam_command_play(wchar_t* playlist_file_path, int a2)
{
	...
	EventHandler::execute_callback<EventHandler::ServerCommandEvent>(server_command,execute_before, ServerConsole::play);
	auto res = p_kablam_command_play(playlist_file_path, a2);
	EventHandler::execute_callback<EventHandler::ServerCommandEvent>(server_command,execute_after, ServerConsole::play);
	return res;
}
