...
bool HandleGuiAccountCreate(char* username, char* email, char* password) {
	...
	if (rtn_code <= 0) {
		...
		if (rtn_code == 0 || rtn_code == ERROR_CODE_CURL_SOCKET_FAILED || rtn_code == ERROR_CODE_CURL_HANDLE || rtn_code == ERROR_CODE_CURL_EASY_PERF
			...
			c_error_menu::open_by_error_id(_cartographer_error_id_account_create_invalid_email);
		}
		else if (rtn_code == ERROR_CODE_INVALID_USERNAME) {
			c_error_menu::open_by_error_id(_cartographer_error_id_account_create_invalid_username);
		}
		else if (rtn_code == ERROR_CODE_INVALID_PASSWORD) {
			c_error_menu::open_by_error_id(_cartographer_error_id_account_create_invalid_password);
		}
		else if (rtn_code == ERROR_CODE_TAKEN_EMAIL) {
			c_error_menu::open_by_error_id(_cartographer_error_id_account_create_email_already_used);
		}
		else if (rtn_code == ERROR_CODE_TAKEN_USERNAME) {
			c_error_menu::open_by_error_id(_cartographer_error_id_account_create_username_taken);
		}
		else if (rtn_code == ERROR_CODE_BANNED_EMAIL_DOMAIN) {
			c_error_menu::open_by_error_id(_cartographer_error_id_account_create_blacklisted_email_provider);
		}
		else {
			//unknown error!
