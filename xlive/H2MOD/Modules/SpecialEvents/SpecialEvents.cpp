...
namespace SpecialEvents
{
	...
	std::vector<std::tuple<byte, byte, float, float, float, float, float, float, float>> lockout_scen_places
	{
	...
	std::vector<std::tuple<byte, byte, float, float, float, float, float, float, float>> coag_scen_places
	{
			...
			if (wcscmp(mapName, L"Coagulation") == 0)
			{
				*Memory::GetAddress<Player::Biped*>(0x51A67C) = Player::Biped::Skeleton;
				for (auto& scen_place : coag_scen_places)
				{
					...
					ori.z = std::get<7>(scen_place);
					set_orientation(&placement.orientation, &placement.up, &ori);
					placement.scale = std::get<8>(scen_place);
