...
void c_account_list_menu::set_menu_open_context(e_cartographer_account_manager_screen_type screen_type)
{
	g_open_cartographer_account_manager_context = screen_type;
}

c_account_list_menu* c_account_list_menu::open_account_list_context()
{
	set_menu_open_context(_cartographer_account_manager_screen_type_list);
	return (c_account_list_menu*)ui_custom_cartographer_load_menu(c_account_list_menu::open, 3);
}

c_account_list_menu* c_account_list_menu::open_account_remove_from_list_context()
{
	set_menu_open_context(_cartographer_account_manager_screen_type_list_remove_account);
	return (c_account_list_menu*)ui_custom_cartographer_load_menu(c_account_list_menu::open, 3);
}

c_account_list_menu* c_account_list_menu::open_account_create_context() 
{
	set_menu_open_context(_cartographer_account_manager_screen_type_create_account);
	return (c_account_list_menu*)ui_custom_cartographer_load_menu(c_account_list_menu::open);
}

c_account_list_menu* c_account_list_menu::open_account_add_context() 
{
...
void* c_account_list_menu::open_default_context(s_screen_parameters* parameters)
{
	set_menu_open_context(_cartographer_account_manager_screen_type_list);
	return open(parameters);
}

void* __cdecl c_account_list_menu::open(s_screen_parameters* parameters)
{
	...
	uint8* ui_buffer = (uint8*)ui_memory_pool_allocate(sizeof(c_account_list_menu), 0);
	...
	if (ui_buffer) {
		account_list_menu = new (ui_buffer) c_account_list_menu(
...
c_account_list_menu::c_account_list_menu(e_user_interface_channel_type _ui_channel, e_user_interface_render_window _window_index, uint16 _flags, e_cartographer_account_manager_screen_type _screen_type, int32 _button_count, int32 _selected_button) :
...
c_account_edit_list::c_account_edit_list(uint16 _flags, int32 _button_count, int32 _default_selected_button, e_cartographer_account_manager_screen_type _screen_type) :
	c_list_widget(_flags),
	field_2C0(0),
	m_slot_2(this, &c_account_edit_list::button_handler)
{
	...
	if ((void*)this == (void*)-offsetof(c_account_edit_list, m_slot_2)) {
	...
	case _cartographer_account_manager_screen_type_create_account:
		...
		get_label(m_cartographer_screen_type, _screen_type_account_create_button_placeholder_username_text, &placeholder_username);
		get_label(m_cartographer_screen_type, _screen_type_account_create_button_placeholder_email_text, &placeholder_email);
		get_label(m_cartographer_screen_type, _screen_type_account_create_button_placeholder_password_text, &placeholder_password);
	...
	case _cartographer_account_manager_screen_type_add_account:
		...
		get_label(m_cartographer_screen_type, _screen_type_account_add_button_placeholder_username_text, &placeholder_username);
		get_label(m_cartographer_screen_type, _screen_type_account_add_button_placeholder_password_text, &placeholder_password);
		...
		*(DWORD*)(a1 + 2652) = NULL;
		...
		if (g_account_manager_master_login_code < 0)
		{
			...
			case ERROR_CODE_INVALID_PARAM:
				c_error_menu::open_by_error_id(_cartographer_error_id_internal_error);
				return;

			case ERROR_CODE_CURL_EASY_PERF:
				c_error_menu::open_by_error_id(_cartographer_error_id_account_login_connection_failed);
				return;
			...
			case ERROR_CODE_INVALID_LOGIN_ID:
				c_error_menu::open_by_error_id(_cartographer_error_id_login_invalid_account_id);
				return;

			case ERROR_CODE_INVALID_PASSWORD:
				c_error_menu::open_by_error_id(_cartographer_error_id_login_incorrect_password);
				return;

			case ERROR_CODE_MACHINE_BANNED:
				c_error_menu::open_by_error_id(_cartographer_error_id_login_machine_banned);
				return;

			case ERROR_CODE_ACCOUNT_BANNED:
				c_error_menu::open_by_error_id(_cartographer_error_id_login_account_banned);
				return;

			case ERROR_CODE_ACCOUNT_DISABLED:
				c_error_menu::open_by_error_id(_cartographer_error_id_login_account_disabled);
				return;

			case ERROR_CODE_MACHINE_SERIAL_INSUFFICIENT:
				c_error_menu::open_by_error_id(_cartographer_error_id_login_insufficient_machine_identifiers);
				return;

			default:
				//unknown error!
				c_error_menu::open_by_error_id(_cartographer_error_id_unknown_unhandled_error);
				return;
			}
		}
		else {
...
void c_account_edit_list::cartographer_type_add_button_handler(s_event_record* event_record, int32* a3)
{
...
void c_account_edit_list::cartographer_type_list_button_handler(s_event_record* event_record, int32* a3)
{
	int button_id = DATUM_INDEX_TO_ABSOLUTE_INDEX(*a3);
	...
	if (button_id == H2AccountCount + 1) // create account
	{
		...
		{
			c_account_list_menu::open_account_create_context();
		}
	}
	...
	{ 
		...
		user_interface_back_out_from_channel(parent_screen_ui_channel, parent_render_window);
		g_account_manager_remove_mode ? c_account_list_menu::open_account_remove_from_list_context() : c_account_list_menu::open_account_list_context();
	}
...
static DWORD WINAPI thread_account_create_proc_cb(LPVOID lParam)
{
	...
	{
		c_error_menu::open_by_error_id(_cartographer_error_id_account_create_verification_email_sent);
...
void c_account_edit_list::cartographer_type_create_account(s_event_record* event_record, int32* a3)
{
	int button_id = DATUM_INDEX_TO_ABSOLUTE_INDEX(*a3);
	...
	else if (button_id == 3) 
	{
		if (!g_account_manager_thread_handle) 
		{
			...
			user_interface_back_out_from_channel(parent_screen_ui_channel, parent_render_window);
			c_error_menu::open_by_error_id(_cartographer_error_id_account_create_processing_account_notice);
...
void c_account_edit_list::button_handler(s_event_record* event_record, int32* a3)
{
	switch (m_cartographer_screen_type)
	{
	case _cartographer_account_manager_screen_type_list:
