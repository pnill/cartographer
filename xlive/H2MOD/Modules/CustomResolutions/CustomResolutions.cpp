...
void CustomResolution::Initialize()
{
	...
	WritePointer((DWORD)h2mod->GetAddress(0x263C84, 0x0), &video_options[0].res_y);
	WritePointer((DWORD)h2mod->GetAddress(0x263CA1, 0x0), &video_options[0].unk);


	WritePointer((DWORD)h2mod->GetAddress(0x26443B, 0x0), video_options);
	WritePointer((DWORD)h2mod->GetAddress(0x264414, 0x0), &video_options[0].res_y);

	WritePointer((DWORD)h2mod->GetAddress(0x263C25, 0x0), video_options);
	WritePointer((DWORD)h2mod->GetAddress(0x263C55, 0x0), &video_options[0].res_y);

	WritePointer((DWORD)h2mod->GetAddress(0x264375, 0x0), video_options);
	WritePointer((DWORD)h2mod->GetAddress(0x26439F, 0x0), &video_options[0].res_y);
