...
bool NetworkSession::getCurrentNetworkSession(network_session** a1)
{
	...
	return reinterpret_cast<get_lobby_globals_ptr>(h2mod->GetAddress(0x1AD736, 0x1A66B3))(a1);
}
...
signed int NetworkSession::getPeerIndexFromNetworkAddress(network_address* addr)
{
	...
	return reinterpret_cast<get_peer_index_from_network_address>(h2mod->GetAddress(0x1C71DF, 0x19E9CF))(getCurrentNetworkSession(), addr);
}
...
char NetworkSession::getMapFileLocation(network_session* thisx, wchar_t* buffer, size_t szBuffer)
{
	...
	return reinterpret_cast<get_map_file_location_impl>(h2mod->GetAddress(0x1C5678, 0x19CD4A))(thisx, buffer, szBuffer);
}
