...
#pragma region Load Configuration
	...
	if (h2f)
	{
		...
		while (!feof(h2f))
		{
			...
			CHECK_ARG("br_turn_off_crosshair_dot = ", settings->br_turn_off_crosshair_dot);
			CHECK_ARG("br_turn_off_contrail = ", settings->br_turn_off_contrail);
			...
			CHECK_ARG_FLOAT("sg_projectiles_per_shot = ", settings->sg_projectiles_per_shot);
