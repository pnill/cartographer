...
namespace FiloInterface
{
	...
	{
		typedef filo* (__cdecl* filo_init_t)(filo*, const char*, bool);
		filo_init_t p_init_filo = Memory::GetAddress<filo_init_t>(0x8C409, 0x86D37);
	...
	bool open(filo* filo_ptr, __int16 mode, DWORD* out_error_code)
	{
		...
		filo_open_t p_filo_open = Memory::GetAddress<filo_open_t>(0x638BF, 0x65BBF);
	...
	bool close(filo* filo_ptr)
	{
		...
		filo_close_t p_filo_close = Memory::GetAddress<filo_close_t>(0x63B66, 0x65E66);
	...
	bool create_file_or_directory(filo* filo_ptr)
	{
		...
		filo_create_file_or_directory_t p_filo_create_file_or_directory = Memory::GetAddress<filo_create_file_or_directory_t>(0x632F1, 0x655F1);
	...
	bool delete_existing(filo* filo_ptr)
	{
		...
		filo_delete_t p_filo_delete = Memory::GetAddress<filo_delete_t>(0x63667, 0x65967);
	...
	bool read(filo* filo_ptr, LPVOID data_buffer, DWORD nNumberOfBytesToRead, bool suppress_errors)
	{
		...
		filo_read_t p_filo_read = Memory::GetAddress<filo_read_t>(0x63C60, 0x65F3C);
	...
	bool write(filo* filo_ptr, LPVOID data, size_t data_size)
	{
		...
		filo_write_t p_filo_write = Memory::GetAddress<filo_write_t>(0x63CBC, 0x65F98);
	...
	bool get_low_file_size(filo* filo_ptr, DWORD* out_low_size)
	{
		...
		filo_get_low_file_size_t p_filo_get_low_file_size = Memory::GetAddress<filo_get_low_file_size_t>(0x63E10, 0x660EC);
	...
	bool set_file_attribute_readonly(filo* filo_ptr, bool read_only)
	{
		...
		filo_set_file_attribute_readonly_t p_filo_set_file_attribute_readonly = Memory::GetAddress<filo_set_file_attribute_readonly_t>(0x6341D, 0x6571D);
	...
	bool set_file_attribute_hidden(filo* filo_ptr, bool hidden)
	{
		...
		filo_set_file_attribute_hidden_t p_filo_set_file_attribute_hidden = Memory::GetAddress<filo_set_file_attribute_hidden_t>(0x63545, 0x65845);
