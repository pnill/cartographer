#pragma once
#include "..\Runtime\Globals\global_ptrs.h"
namespace Blam
{
	namespace Cache
	{
...
void Blam::Cache::DataTypes::Reflexive<T>::Open()
{
	...
	this->TagBlockOffset = (DWORD)tptr - *(DWORD*)Runtime::Globals::GlobalTagBase;
...
{
	return GetFieldSize() * TagBlockCount;
}
template<typename T>
inline void* Blam::Cache::DataTypes::Reflexive<T>::GetTagBlockElements()
{	
	DWORD MemPtr = *(DWORD*)Runtime::Globals::GlobalTagBase + this->TagBlockOffset;
	void* TagFields = (void*)MemPtr;
	return TagFields;
}
template<typename T>
