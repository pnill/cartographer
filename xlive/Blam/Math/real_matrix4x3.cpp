...
void matrix4x3_rotation_to_quaternion(const real_matrix4x3* matrix, real_quaternion* out_quat)
{
	...
	matrix4x3_rotation_to_quaternion_t p_matrix4x3_rotation_to_quaternion = Memory::GetAddressRelative<matrix4x3_rotation_to_quaternion_t>(0x4791A2);
	return p_matrix4x3_rotation_to_quaternion(matrix, out_quat);
}

void matrix4x3_from_orientation(const real_quaternion* quat_rot, real_matrix4x3* out_m)
{
	typedef void(__cdecl* matrix4x3_from_orientation_t)(real_matrix4x3*, const real_quaternion*);
	matrix4x3_from_orientation_t p_matrix4x3_from_orientation = Memory::GetAddressRelative<matrix4x3_from_orientation_t>(0x4778CB);
	return p_matrix4x3_from_orientation(out_m, quat_rot);
}
...
void quaternion_interpolate_and_normalize(const real_quaternion* previous, const real_quaternion* current, float fractional_ticks, real_quaternion* out_quaternion)
{
	...
	quaternion_interpolate_and_normalize_t p_quaternion_interpolate_and_normalize = Memory::GetAddressRelative<quaternion_interpolate_and_normalize_t>(0x434F6F);
...
void matrix4x3_interpolate(const real_matrix4x3* previous, const real_matrix4x3* target, float fractional_ticks, real_matrix4x3* out_mat)
{
	...
	matrix4x3_from_orientation(&q3_interpolated, out_mat);
