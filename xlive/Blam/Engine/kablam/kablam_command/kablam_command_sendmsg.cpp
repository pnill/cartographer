...
kablam_command* kablam_command_sendmsg::create_instance(wchar_t** arguments, uint32 argument_count, kablam_string* out_message)
{
	out_message->free();
	...
	if (wcsnlen(arguments[1], 121) == 121)
	{
		out_message->load(kablam_string_err_text_too_long);
		return nullptr;
	}

	kablam_command_sendmsg* instance = new kablam_command_sendmsg();

	instance->type = _kablam_command_send_message;
	instance->valid = true;
	instance->result_code = send_message_result_code_sent;
	wcsncpy_s(instance->message, 121, arguments[1], -1);
