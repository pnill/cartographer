...
kablam_command* kablam_command_players::create_instance(wchar_t** arguments, uint32 argument_count,	kablam_string* out_message)
{
	...
	if (argument_count == 1)
	{
		instance->type = _kablam_command_players_get;
		instance->max_players = 0;
	}
	else
	{
		instance->type = _kablam_command_players_set;
