...
bool c_simulation_entity_database::process_creation(int32 entity_index, e_simulation_entity_type type, uint32 update_mask, int32 block_count, s_replication_allocation_block* blocks)
{
    ...
    // clear the block data, for some reason??
...
uint32 c_simulation_entity_database::read_creation_from_packet(int32 entity_index, e_simulation_entity_type* simulation_entity_type, uint32* out_update_mask, int32 a5, int32* block_count, s_replication_allocation_block* blocks, c_bitstream* packet)
{
    uint32 result = 3;
                    ...
                    (s_simulation_baseline_state_data*)state_data))
            {
                ...
                if (decode_success)
                {
                    ...
                    *out_update_mask = entity_out_update_mask;
                    ...
                    blocks[_entity_creation_block_order_forward_memory_queue_element].block_type = _network_memory_block_forward_simulation_queue_element;
                    blocks[_entity_creation_block_order_forward_memory_queue_element].block_size = sizeof(queue_element);
                    blocks[_entity_creation_block_order_forward_memory_queue_element].block_data = queue_element;

                    blocks[_entity_creation_block_order_simulation_entity_creation].block_type = _network_memory_block_simulation_entity_creation;
                    blocks[_entity_creation_block_order_simulation_entity_creation].block_size = creation_data_size;
                    blocks[_entity_creation_block_order_simulation_entity_creation].block_data = creation_data;

                    blocks[_entity_creation_block_order_simulation_entity_state].block_type = _network_memory_block_simulation_entity_state;
                    blocks[_entity_creation_block_order_simulation_entity_state].block_size = state_data_size;
                    blocks[_entity_creation_block_order_simulation_entity_state].block_data = state_data;
