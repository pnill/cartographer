...
/* Returns success */
bool __cdecl file_close(struct s_file_reference* file_reference);

/* Deletes the file or directory pointed to by the s_file_reference, returns success */
bool __cdecl file_create(struct s_file_reference* file_reference);

/* Returns true if the path exists and we can access it */
bool __cdecl file_delete(struct s_file_reference* file_reference);

bool file_exists(const struct s_file_reference* file);

bool file_set_position(struct s_file_reference* file, size_t position, bool silent);

uint32 file_get_eof(const struct s_file_reference* file);

/* */
bool file_set_eof(struct s_file_reference* file, size_t position);
...
/* */
bool __cdecl file_get_size(struct s_file_reference* file_reference, uint32* size);

/* Can be used to truncate or extend an open file, returns success */
bool file_change_size(struct s_file_reference* file_reference, int32 new_size);

/* */
bool __cdecl file_read_only(struct s_file_reference* file_reference, bool read_only);

/* */
bool __cdecl file_set_hidden(struct s_file_reference* file_reference, bool hidden);
