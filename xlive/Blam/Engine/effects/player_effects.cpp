        ...
        else
        {
            ...
            if (user_effect->camera_shake_countdown > 0 || apply_camera_shake)
            {
                ...
                real32 periodic_function_result_scaled = 
                    transition_function_result * ((1.0f - user_effect->camera_shaking.wobble_weight) + (user_effect->camera_shaking.wobble_weight * periodic_function_result));

                real32 v1 = MAX(0.0f, user_effect->camera_shaking.random_translation * periodic_function_result_scaled);
                real32 v2 = MAX(0.0f, user_effect->camera_shaking.random_rotation * periodic_function_result_scaled);
...
void player_effect_apply_camera_effect_matrix_internal(real_matrix4x3* matrix, real32 a2, real32 a3)
{
    ...
    {
        _random_direction3d(get_local_random_seed_address(), NULL, __FILE__, __LINE__, &vector);
        scale_vector3d(&vector, a2, &matrix->position);
    }
    return;
}
