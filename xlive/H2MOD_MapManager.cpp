...
DWORD mapFilenameLoc2 = 0;

void onSingleMapLoadCave() {
	char* mapFilenameLoaded2 = (char*)(mapFilenameLoc2);
	TRACE_GAME_N("[h2mod-mapmanager] Map loaded %s", mapFilenameLoaded2);
}

__declspec(naked) void onSingleMapLoaded(void) {
	__asm {
...
typedef char(__stdcall *read_map_set_registry)(char* thisx);
read_map_set_registry read_map_set_registry_method;
char __stdcall readMapSetRegistry(char* thisx) {
	WORD *v1; // edi@1
	struct _RTL_CRITICAL_SECTION *v2; // esi@1
	char v3; // bl@1
	char result; // al@2
	HKEY phkResult; // [sp+10h] [bp-24250h]@1
	DWORD reg_key_data_size; // [sp+14h] [bp-2424Ch]@1
	DWORD Type; // [sp+18h] [bp-24248h]@1
	struct _RTL_CRITICAL_SECTION *v8; // [sp+1Ch] [bp-24244h]@1
	BYTE reg_key_data[4]; // [sp+20h] [bp-24240h]@1
	int v10; // [sp+24h] [bp-2423Ch]@1
	//0x24228 original value
	char v11[0x48470]; // [sp+28h] [bp-24238h]@1
	int v12; // [sp+2425Ch] [bp-4h]@1

	v1 = (WORD*)thisx;
	v8 = *(struct _RTL_CRITICAL_SECTION **)thisx;
	v2 = v8;
	...
	phkResult = 0;
	Type = 0;
	*(DWORD *)reg_key_data = 0;
	v10 = 0;
	memset(&v11, 0, 0x24228 * 2);
	reg_key_data_size = 0x24230 * 2;
	typedef char(*sub_1334541_type)();
	sub_1334541_type sub_1334541 = (sub_1334541_type)(h2mod->GetBase() + 0x4541);
	if (sub_1334541() || RegOpenKeyExA(HKEY_CURRENT_USER, "Software\Microsoft\Halo 2", 0, 0x20019u, &phkResult))
	{
	...
	{
		if (!RegQueryValueExA(phkResult, "Map Set", 0, &Type, reg_key_data, &reg_key_data_size) && Type == 3)
		{
			typedef char(__thiscall *touch_map_limit2_type)(void* thisx, int registry_value, unsigned int a3);
			touch_map_limit2_type touch_map_limit2 = (touch_map_limit2_type)(h2mod->GetBase() + 0xC23BF);
			if (touch_map_limit2(v1, (int)reg_key_data, reg_key_data_size))
			{
				typedef char(__thiscall *read_map_data_from_reg_key_type)(WORD* thisx);
				read_map_data_from_reg_key_type read_map_data_from_reg_key = (read_map_data_from_reg_key_type)(h2mod->GetBase() + 0xC2A62);
				read_map_data_from_reg_key(v1);
				v3 = 1;
			}
		}
...
typedef char(__stdcall *write_map_set_registry)(char* thisx);
write_map_set_registry write_map_set_registry_method;

char __stdcall writeMapSetRegistry(char* thisx) {
	char *v1; // edi@1
	struct _RTL_CRITICAL_SECTION *v2; // esi@1
	char v3; // bl@1
	DWORD cbData; // [sp+10h] [bp-2424Ch]@1
	HKEY phkResult; // [sp+14h] [bp-24248h]@1
	struct _RTL_CRITICAL_SECTION *v7; // [sp+18h] [bp-24244h]@1
	BYTE Data[4]; // [sp+1Ch] [bp-24240h]@1
	int v9; // [sp+20h] [bp-2423Ch]@1
	//old value 0x24238
	char v10[0x48470]; // [sp+24h] [bp-24238h]@1
	int v11; // [sp+24258h] [bp-4h]@1

	v1 = thisx;
	v7 = *(struct _RTL_CRITICAL_SECTION **)thisx;
	v2 = v7;
	...
	*(DWORD *)Data = 0;
	v9 = 0;
	memset(&v10, 0, (0x24228 * 2));
	cbData = (0x24230 * 2);                             // offset to the custom maps location on the file system
	typedef char(*sub_1334541_type)();
	sub_1334541_type sub_1334541 = (sub_1334541_type)(h2mod->GetBase() + 0x4541);
	if ((unsigned __int8)sub_1334541())
		goto LABEL_2;

	typedef char(__thiscall *writes_to_map_memory_type)(int thisx, int a2, DWORD* a3);
	writes_to_map_memory_type writes_to_map_memory = (writes_to_map_memory_type)(h2mod->GetBase() + 0xC223A);

	if (writes_to_map_memory((int)v1, (int)Data, &cbData))
	{
		if (RegCreateKeyExA(HKEY_CURRENT_USER, "Software\Microsoft\Halo 2", 0, 0, 0, 0x20006u, 0, &phkResult, 0))
		{
		LABEL_2:
			LeaveCriticalSection(v2);
			return 0;
		}
		cbData = RegSetValueExA(phkResult, "Map Set", 0, 3u, Data, cbData);
		RegCloseKey(phkResult);
		if (!cbData)
		{
			typedef char(__thiscall *read_map_limit8_type)(char* thisx);
			read_map_limit8_type read_map_limit8 = (read_map_limit8_type)(h2mod->GetBase() + 0xC28B9);
			if (read_map_limit8(v1))
				v3 = 1;
		}
	}
	LeaveCriticalSection(v2);
...
void MapManager::increaseMapLimit() {
	//TODO: use later to increase maps read
	DWORD* mapsObject = (DWORD*)(h2mod->GetBase() + (h2mod->Server ? 0x4A70D8 : 0x482D70));
	
	BYTE bytes[1] = { 0x64 };
	//in h2_touch_limit2 (0xC23BF), there is a cmp where if map count is less than 50 certain code doesn't execute
	//patchBYTEs((BYTE*)h2mod->GetBase() + 0xC23D9, bytes, 1);

	//also in h2_touch_limit we read the map limit from map set from the map set registry value and we write out the map set as well
	//so we need to increase the map set variable as well, the original value is 24220h
	//new value is 48440
	//BYTE bytes2[3] = { 0x40, 0x84, 0x04};
	//TODO: can't increase this memset without increasing the memory its memsetting
	//patchBYTEs((BYTE*)h2mod->GetBase() + 0xC2412, bytes2, 3);

	//in h2_custom_game_custom_maps (0x25AE3B) the map limit is
	//patchBYTEs((BYTE*)h2mod->GetBase() + 0x25AF4B, bytes, 1);

	//in h2_reads_custom_map_names (0x4CF8D) the map limit is referenced
	//patchBYTEs((BYTE*)h2mod->GetBase() + 0x4CFAB, bytes, 1);

	//in h2_touch_map_limit3, there is a cmp against map count vs 50
	//patchBYTEs((BYTE*)h2mod->GetBase() + 0xC25F1, bytes, 1);

	//for debuging all the maps loaded into memory
	Codecave(h2mod->GetBase() + 0x4D181, onSingleMapLoaded, 0);

	//patchBYTEs((BYTE*)h2mod->GetBase() + 0x592A6, bytes, 1);

	//memset(customMapBuffer, 0, 6000 * (sizeof DWORD));
	//swap(mapsObject, customMapBuffer);

	DWORD dwBack = 0;
	//write_map_set_registry_method = (write_map_set_registry)DetourClassFunc((BYTE*)h2mod->GetBase() + 0xC2D4D, (BYTE*)writeMapSetRegistry, 13);
	//VirtualProtect(write_map_set_registry_method, 4, PAGE_EXECUTE_READWRITE, &dwBack);

	//read_map_set_registry_method = (read_map_set_registry)DetourClassFunc((BYTE*)h2mod->GetBase() + 0xC2D4D, (BYTE*)readMapSetRegistry, 13);
	//VirtualProtect(read_map_set_registry_method, 4, PAGE_EXECUTE_READWRITE, &dwBack);
